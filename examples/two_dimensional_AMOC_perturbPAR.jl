...
POPₙ = @lift 1e3*interior(POP_timeseries[$n], 1, :, 150:200)
avg_POPₙ = @lift mean(1e3*interior(POP_timeseries[$n], 1, :, :), dims=1) 

NCPₙ = @lift 1e3*1day*interior(NCP_timeseries[$n], 1, :, 150:200)
avg_NCPₙ = @lift mean(1e3*1day*interior(NCP_timeseries[$n], 1, :, :), dims=1) 

Preminₙ = @lift 1e3*1day*interior(Premin_timeseries[$n], 1, :, 150:200)
avg_Preminₙ = @lift mean(1e3*1day*interior(Premin_timeseries[$n], 1, :, :), dims=1) 
...
# And, finally, we record a movie.
frames = 1:length(times)
record(fig_rates, "AMOC115_1N12perY_rate.mp4", frames, framerate=50) do i
    n[] = i
    kz_prof[1]= kz_profiles[:,i]
    NCP_up[1] = up_NCPₙ[]
