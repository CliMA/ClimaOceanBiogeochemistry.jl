...
using JLD2
data = JLD2.load("CAN_2D_Rz12.jld2")
...
simulation2 = Simulation(model2; Δt = 5minutes, stop_time=365.25*100days) 
...
outputs2 = (u = model2.velocities.u,
            ...
            DIC = model2.tracers.DIC,
            ALK = model2.tracers.ALK,
            avg_PO₄ = Average(model2.tracers.PO₄, dims=(1, 2)),
            avg_DOP = Average(model2.tracers.DOP, dims=(1, 2)),
            avg_POP = Average(model2.tracers.POP, dims=(1, 2)))

simulation2.output_writers[:simple_output] =
    JLD2OutputWriter(model2, outputs2, 
                     schedule = TimeInterval(7days), 
                     filename = "CAN_2D_Rz12_reset",
...
hm_PO4 = heatmap!(ax_PO4, xw, zw, PO4ₙ; colorrange = (0,3),colormap = :jet1) 
Colorbar(fig[3, 2], hm_PO4; flipaxis = false)

ax_POP = Axis(fig[3, 3]; xlabel = "x (m)", ylabel = "z (m)", title = "[POP] (μM)", aspect = 1)
hm_POP = heatmap!(ax_POP, xw, zw, POPₙ; colorrange = (0,0.04),colormap = :jet1) 
...
record(fig, "CAN_2D_Rz12_reset.mp4", frames, framerate=50) do i
    n[] = i
end
nothing #hide
