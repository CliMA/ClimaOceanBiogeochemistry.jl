...
maxNCP(y,z) = (5e-7 + sinpi(y/Ly) * 5e-5)/day
...
model = HydrostaticFreeSurfaceModel(grid = grid,
                                    biogeochemistry = CarbonAlkalinityNutrients(; grid,
                                                                        maximum_net_community_production_rate  = maximum_net_community_production_rate, #1e-5/day,
...
simulation.output_writers[:simple_output] =
        JLD2OutputWriter(model, outputs, 
                        schedule = TimeInterval(3652.5days), 
                        filename = "AMOC18",
...
record(fig, "AMOC18.mp4", frames, framerate=50) do i
    n[] = i
