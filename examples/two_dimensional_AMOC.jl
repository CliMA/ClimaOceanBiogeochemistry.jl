...
model = HydrostaticFreeSurfaceModel(; grid,
                                    ...
                                    #                                     particulate_organic_phosphorus_sedremin_timescale= 100 / day,
...
set!(model, b=bᵢ) #, DIC=2.1, ALK=2.35, NO₃=2e-2, PO₄=1.5e-3, DOP=0, POP=0, Fe = 8e-7) # mol PO₄ m⁻³
...
record(fig, "AMOC4.mp4", frames, framerate=50) do i
    n[] = i
...
hm_POP = heatmap!(ax_POP, xw/1e3, zw, POP_final; colorrange = (0,0.003),colormap = :jet1) 
...
hm_DOP = heatmap!(ax_DOP, xw/1e3, zw, DOP_final; colorrange = (0,0.03),colormap = :jet1) 
...
hm_NCP = heatmap!(ax_NCP, xw, zw, 1000*NCP_final; colorrange = (0,3e-4),colormap = :jet1) 
...
hm_remin = heatmap!(ax_remin, xw, zw, Remin_final; colorrange = (0,1e-5),colormap = :jet1) 
