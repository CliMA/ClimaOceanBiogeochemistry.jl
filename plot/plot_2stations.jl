...
for i in 1:365
    ...
    Oligo_eratio1[i, 1] = Oligo_POP_flux1[i, 195]/sum(dz*1days*1e3*interior(NCP_timeseries1[i], 1, 490, :))
    Oligo_eratio2[i, 1] = Oligo_POP_flux2[i, 195]/sum(dz*1days*1e3*interior(NCP_timeseries2[i], 1, 490, :))
    Oligo_eratio3[i, 1] = Oligo_POP_flux3[i, 195]/sum(dz*1days*1e3*interior(NCP_timeseries3[i], 1, 490, :))
...
# display(fig)
#
ax_t_prod = Axis(fig[1, 1]; ylabel = "Ratio of F₁₀₀₀/F₂₀₀", xlabel = "t (day)", title = "Transfer effeciency (productive)")
lines!(ax_t_prod, 1:1:365, vec(Prod_Teff1), linewidth = 2, label = "Year")
lines!(ax_t_prod, 1:1:365, vec(Prod_Teff2), linewidth = 2, label = "Year+Month")
...
axislegend(ax_t_prod, position = :rb)
ylims!(ax_t_prod, 0.28, 0.43)

ax_t_oligo = Axis(fig[2, 1]; ylabel = "Ratio of F₁₀₀₀/F₂₀₀", xlabel = "t (day)", title = "Transfer effeciency (oligotrophic)")
lines!(ax_t_oligo, 1:1:365, vec(Oligo_Teff1), linewidth = 2, label = "Year")
lines!(ax_t_oligo, 1:1:365, vec(Oligo_Teff2), linewidth = 2, label = "Year+Month")
