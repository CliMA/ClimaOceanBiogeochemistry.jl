...
# 4.e-3/365.25 
μ(y,z)=  4.e-3/365.25 # 2e-5 + sinpi(y/Ly) * 8e-5  # mol P m⁻³ d⁻¹
set!(μ_surf, μ)   
...
hm_remin = heatmap!(ax_remin, yt/1e3, zt[190:200], POP_remin_final[:,190:200]; colorrange = (0,4e-4),colormap = :viridis) 
Colorbar(fig_rate[2, 2], hm_remin; flipaxis = false)

total_NCP = sum(NCP_final; dims = 2).*Δz # mmol m⁻² d⁻¹
ax_total_NCP = Axis(fig_rate[1, 3]; xlabel = "y (km)", ylabel = "Rate (mmol m⁻² d⁻¹)", title = "Integrated NCP")
ylims!(ax_total_NCP, 0, 2)
lines!(ax_total_NCP, yt/1e3, vec(total_NCP),linewidth = 2)

total_R = sum((total_remin_final); dims = 2).*Δz 
ax_total_R = Axis(fig_rate[2, 3]; xlabel = "y (km)", ylabel = "Rate (mmol m⁻² d⁻¹)", title = "Integrated (POP + DOP) Remin")
ylims!(ax_total_R, 0, 2)
